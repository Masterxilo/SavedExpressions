System`HoldComplete[paul`TokenizeNestedBracePairs::usage = "TokenizeNestedBra\
cePairs[text] splits text into a nested list of strings,\nof the form {\"text \
with no braces\", {\"[\", {...}, \"]\"}, \"text with no braces\", {\"(\", \
{...}, \")\"}, ... etc}.\nHandles [] () and {}, c.f. \
TokenizeNestedBracePairsBraces."; paul`TokenizeNestedBracePairsBraces[] = 
   {"[" -> "]", "{" -> "}", "(" -> ")"}; 
    ...
    System`While[Global`rest != "", Global`op = paul`FirstStringPosition[
       ...
       System`Which[Global`op < Global`cp, System`AppendTo[Global`result, 
         ...
         {Global`innerResult, Global`rest} = paul`TokenizeNestedBracePairs[
           Global`rest, Global`cpString]; Global`rest = System`StringDrop[
           Global`rest, 1]; System`AppendTo[Global`result, 
          {Global`opString, Global`innerResult, Global`cpString}]; , 
        Global`cp < System`Infinity, System`AppendTo[Global`result, 
          System`StringTake[Global`rest, Global`cp - 1]]; 
         Global`rest = System`StringTake[Global`rest, {Global`cp, -1}]; 
         System`Return[{Global`result, Global`rest}], System`True, 
        System`AppendTo[Global`result, Global`rest]; Global`rest = ""; ]]; 
     {Global`result, ""}]; paul`TokenizeNestedBracePairs[
    Global`s_System`String] := System`First[paul`TokenizeNestedBracePairs[
     Global`s, ""]]; paul`MakeUndefinedFunction[
   paul`TokenizeNestedBracePairs]; ]
