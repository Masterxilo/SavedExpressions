System`Cell[System`BoxData[System`RowBox[
   {System`RowBox[{"PRedefinePublicFunction", "[", "\n", 
      System`RowBox[{System`RowBox[{"GridDataMakeFromFiniteMapping", "[", 
          System`RowBox[{"dataNames_List", ",", " ", "f_FiniteMapping"}], 
          "]"}], ",", "\"internal form\"", ",", "\n", "  ", 
        System`RowBox[{"GridData", "[", System`RowBox[{"dataNames", ",", " ", 
            "f"}], "]"}]}], "\n", "  ", System`RowBox[
       {"(*", " ", System`RowBox[{"TODO", " ", "optimize", " ", "the", " ", 
          "case", " ", "of", " ", "NumericQ", " ", "data", " ", 
          System`RowBox[{"and", "/", "or"}], " ", "sparsity", "\n", "TODO", 
          " ", "check", " ", "that", " ", "the", " ", "FiniteMapping", " ", 
          "always", " ", "returns", " ", "a", " ", "List", " ", "of", " ", 
          "data", " ", "of", " ", "always", " ", "the", " ", "same", " ", 
          "length"}], "*)"}], "\n", "\n", "  ", "]"}], ";"}]], "Code", 
 System`GeneratedCell -> System`True, System`CellAutoOverwrite -> 
  System`True, System`CellChangeTimes -> {{3.6815682608556404*^9, 
   3.681568263299692*^9}, {3.681568309008765*^9, 3.681568337522351*^9}, 
   {3.681568370509532*^9, 3.681568449571232*^9}, {3.6815735054759827*^9, 
   3.68157351190701*^9}, 3.681648869270708*^9}]
