System`HoldComplete[Global`NeedsDefined[
   ...
   paul`UnprotectClearAll, Global`a, Scene2D`d, Scene2D`doriginal, SceneX`eg, 
   SceneX`er, SceneX`es, SceneX`ea, Global`c, Global`l, 
   ...
   PackagePackage`Private`x, numerics`Norm2]; 
  PackageDeveloper`RedefinePublicFunction[SceneX`SceneXEnergyf[
...
model,\ndefined by lightIntensityParametrized[l,n]", 
   paul`UnprotectClearAll[Global`a, Scene2D`d, Scene2D`doriginal, SceneX`eg, 
    ...
    System`Module[{SceneX`forwardDifference, SceneX`backwardDifference, 
      ...
      SceneX`norm}, (SceneX`forwardDifference[Scene2D`f_] := 
       ...
       SceneX`backwardGradient[Scene2D`f_] := SceneX`gradient[Scene2D`f, 
        ...
        System`Total[SceneX`gradient[Scene2D`f, SceneX`secondDifference][
           #1]] & ; SceneX`norm[PackagePackage`Private`x_] := 
        System`Sqrt[numerics`Norm2[PackagePackage`Private`x]]; ); 
      SceneX`normalize[PackagePackage`Private`x_] := PackagePackage`Private`x/
        SceneX`norm[PackagePackage`Private`x + System`Table[1.*^-7, 
      ...
      SceneX`gamma[Global`p_] := SceneX`cv[Global`p]/(Global`i[Global`p] + 
         1); SceneX`trob = 3; SceneX`phi[PackagePackage`Private`x_] := 
       1/(1 + SceneX`trob*PackagePackage`Private`x)^3; 
      SceneX`normal[Scene2D`f_] := SceneX`normalize[
         SceneX`forwardGradient[Scene2D`f][#1]] & ; Persist`n[Global`v_] := 
       SceneX`normal[Scene2D`d][Global`v]; 
