System`Cell[System`BoxData[System`RowBox[{"PTagSet", "[", 
    System`RowBox[{"SOPD`SparseOptimizationProblemDecomposed", ",", 
      "\[IndentingNewLine]", System`RowBox[
       {"(*", System`RowBox[{"inspired", " ", "from", " ", 
          "Optimization`UnconstrainedProblems`GetFindMinimumProblem"}], 
        "*)"}], "\[IndentingNewLine]", System`RowBox[
       {System`RowBox[{"SOPD`SparseOptimizationProblemDecomposed", "~", 
          "SetAttributes", "~", "HoldAllComplete"}], ";", 
        "\[IndentingNewLine]", System`RowBox[{"(*", System`RowBox[
           {"stores", " ", "all", " ", "data", " ", "necessary", " ", "for", 
            " ", "describing", " ", "a", " ", "SparseOptimization", " ", 
            "problem", " ", "in", " ", "the", " ", "format", " ", "covered", 
            " ", "here"}], "*)"}], "\[IndentingNewLine]", 
        System`RowBox[{System`RowBox[{"Format", "[", System`RowBox[
             {System`RowBox[{"SOPD`SparseOptimizationProblemDecomposed", "[", 
                "data___", "]"}], ",", "StandardForm"}], "]"}], ":=", 
          System`RowBox[{"\"SOPD\"", "[", System`RowBox[{"Short", "@", 
              System`RowBox[{"{", "data", "}"}]}], "]"}]}]}]}], 
    "\[IndentingNewLine]", "]"}]], "Input", System`CellChangeTimes -> 
  {3.6803826081470222*^9, {3.680383243131374*^9, 3.6803832454192286*^9}, 
   {3.6809334930287733*^9, 3.680933496812441*^9}, {3.6809335468029633*^9, 
   3.6809335612829485*^9}, {3.6809340681053534*^9, 3.6809341249819345*^9}, 
   {3.681194596410492*^9, 3.681194615343686*^9}}, 
 System`CellTags -> "depersistedCellfc78da23-cfb2-4113-b472-2bbf560cc2e4", 
 System`CellID -> 145613957]
