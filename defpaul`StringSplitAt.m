System`HoldComplete[paul`UnprotectClearAll[paul`StringSplitAt]; 
  paul`StringSplitAt::usage = "split a string at a certain position, \
optionally keep the character\nat the split point with left or right string \
with the option \"KeepSeparator\" -> \"Left\"|\"Right\",\ndefault is None"; 
  System`Options[paul`StringSplitAt] = {"KeepSeparator" -> System`None}; 
  ...
  paul`MakeUndefinedFunction[paul`StringSplitAt]; 
  System`Protect[paul`StringSplitAt]; ]
