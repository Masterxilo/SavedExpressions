System`Cell[System`BoxData[System`RowBox[
   {System`RowBox[{"PRedefinePublicFunction", "[", "\n", 
      System`RowBox[{System`RowBox[{"GDAsRulesOfRules", "[", 
          System`RowBox[{"g", " ", ":", " ", System`RowBox[{"GridData", "[", 
              System`RowBox[{"dataNames_List", ",", " ", "f_FiniteMapping"}], 
              "]"}]}], "]"}], "\n", "  ", ",", 
        "\"rules of lists of rules,\n position -> dataName -> value\"", "\n", 
        "  ", ",", System`RowBox[{"MapAt", "[", "\n", "  ", 
          System`RowBox[{System`RowBox[{System`RowBox[{"Thread", "@", 
                System`RowBox[{"Rule", "[", System`RowBox[{"dataNames", ",", 
                    " ", "#"}], "]"}]}], " ", "&"}], ",", "\n", "  ", 
            System`RowBox[{"GDAsRules", "@", "g"}], ",", "\n", "  ", 
            System`RowBox[{"{", System`RowBox[{"All", ",", " ", "2"}], 
              "}"}]}], "\n", "  ", "]"}]}], "\n", "]"}], ";"}]], "Code", 
 System`CellChangeTimes -> {{3.6815682608556404*^9, 3.681568263299692*^9}, 
  {3.681568309008765*^9, 3.681568337522351*^9}, {3.681568370509532*^9, 
  3.681568449571232*^9}, {3.6815735054759827*^9, 3.68157351190701*^9}, 
  {3.681573561428832*^9, 3.6815735943078794*^9}, {3.681573675570326*^9, 
  3.6815738510684156*^9}, {3.681574105049485*^9, 3.6815741711690283*^9}}]
