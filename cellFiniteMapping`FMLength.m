System`Cell[System`BoxData[System`RowBox[
   {"\n", System`RowBox[{"PRedefinePublicFunctionAlternatives", "[", "\n", 
      System`RowBox[{"\"|A|\"", "\n", "\n", ",", System`RowBox[
         {"{", System`RowBox[{System`RowBox[{"FMLength", "[", 
              System`RowBox[{"f", " ", ":", " ", System`RowBox[
                 {"FiniteMapping", "[", System`RowBox[{"Array", ",", " ", 
                    "level_", ",", " ", "array_"}], "]"}]}], "]"}], ",", 
            System`RowBox[{"Times", "@@", System`RowBox[{System`RowBox[
                 {"Dimensions", "[", "array", "]"}], "[", System`RowBox[
                 {"[", System`RowBox[{"1", ";;", "level"}], "]"}], "]"}]}]}], 
          "}"}], "\n", ",", System`RowBox[{"{", System`RowBox[
           {System`RowBox[{"FMLength", "[", "f_FiniteMapping", "]"}], ",", 
            System`RowBox[{"Length", "@", System`RowBox[{"FMDomain", "@", 
                "f"}]}]}], "}"}]}], "\n", "]"}]}]], "Code", 
 System`CellChangeTimes -> {{3.6815674723302517*^9, 3.68156758311055*^9}, 
  {3.681567682990239*^9, 3.6815677013030987*^9}, {3.6815677352214437*^9, 
  3.681567891021942*^9}, {3.681567959368266*^9, 3.681568076260194*^9}, 
  {3.6815684462000237*^9, 3.6815684895511427*^9}}]
