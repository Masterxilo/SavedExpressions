System`Cell[System`BoxData[System`RowBox[
   {System`RowBox[{"PRedefinePublicFunction", "[", "\n", 
      System`RowBox[{System`RowBox[{"GDSubset", "[", System`RowBox[
           {System`RowBox[{"GridData", "[", System`RowBox[{"dataNames_List", 
                ",", " ", "f_FiniteMapping"}], "]"}], ",", " ", 
            System`RowBox[{"positions", " ", ":", " ", System`RowBox[{"{", 
                System`RowBox[{System`RowBox[{"{", "__Integer", "}"}], 
                  ".."}], "}"}]}]}], "]"}], "\n", "\n", ",", 
        "\"Create a new grid from a subset of the positions.\"", "\n", "\n", 
        ",", System`RowBox[{System`RowBox[{"{", System`RowBox[
             {"g", "=", System`RowBox[{"FMDomainSubset", "[", System`RowBox[
                 {"f", ",", " ", "positions"}], "]"}]}], "}"}], "~", "With", 
          "~", "\n", "    ", System`RowBox[{"GridDataMakeFromFiniteMapping", 
            "[", System`RowBox[{"dataNames", ",", " ", "g"}], "]"}]}]}], 
      "\n", "]"}], ";"}]], "Code", System`GeneratedCell -> System`True, 
 System`CellAutoOverwrite -> System`True, System`CellChangeTimes -> 
  {{3.6815682608556404*^9, 3.681568263299692*^9}, {3.681568309008765*^9, 
   3.681568337522351*^9}, {3.681568370509532*^9, 3.681568449571232*^9}, 
   {3.6815735054759827*^9, 3.68157351190701*^9}, {3.681573561428832*^9, 
   3.6815735943078794*^9}, {3.681573675570326*^9, 3.6815736959898186*^9}, 
   3.6816352944795074*^9}]
