System`Cell[System`BoxData[System`RowBox[{"Block", "[", 
    System`RowBox[{System`RowBox[{"{", "F", "}"}], ",", 
      "\[IndentingNewLine]", System`RowBox[{"PTest", "[", 
        "\[IndentingNewLine]", System`RowBox[
         {System`RowBox[{System`RowBox[{System`RowBox[{"F", "[", "x_", "]"}], 
              ":=", "0"}], ";", "\[IndentingNewLine]", System`RowBox[
             {System`RowBox[{"F", "[", "2", "]"}], "//", 
              "WhichDownValueAndPatternVariableValues"}]}], ",", 
          "\[IndentingNewLine]", System`RowBox[{"{", System`RowBox[
             {System`RowBox[{"HoldPattern", "[", System`RowBox[{"F", "[", 
                  "x_", "]"}], "]"}], ",", System`RowBox[{"{", System`RowBox[
                 {"x", "\[Rule]", "2"}], "}"}]}], "}"}]}], 
        "\[IndentingNewLine]", "]"}]}], "\[IndentingNewLine]", "]"}]], 
 "Input", System`CellChangeTimes -> {{3.681663947881822*^9, 
  3.6816639740615053*^9}, {3.6816640057310443*^9, 3.6816640154091215*^9}}]
