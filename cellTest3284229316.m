System`Cell[System`BoxData[System`RowBox[{"Block", "[", 
    System`RowBox[{System`RowBox[{"{", System`RowBox[{"id", ",", "id2"}], 
        "}"}], ",", System`RowBox[{"PTest", "[", System`RowBox[
         {System`RowBox[{System`RowBox[{"(", System`RowBox[{System`RowBox[
                 {"id", "=", System`RowBox[
                   {"RIFunctionMakeFromExpressionList", "[", System`RowBox[
                     {System`RowBox[{"{", "x", "}"}], ",", System`RowBox[
                       {"{", "x", "}"}]}], "]"}]}], ";"}], ")"}], ";", 
            System`RowBox[{"(", System`RowBox[{System`RowBox[{"id2", "=", 
                  System`RowBox[{"RIFunctionMakeComposition", "[", 
                    System`RowBox[{"id", ",", "id"}], "]"}]}], ";"}], ")"}], 
            ";", System`RowBox[{"{", System`RowBox[{System`RowBox[
                 {"FMAsRules", "[", System`RowBox[
                   {"RIFunctionOutputExpressionMap", "[", "id2", "]"}], 
                  "]"}], ",", System`RowBox[{"RVVAsRules", "[", System`RowBox[
                   {"RIFunctionEvaluate", "[", System`RowBox[{"id2", ",", 
                      System`RowBox[{"RVarvalMake", "[", System`RowBox[
                         {"{", System`RowBox[{"x", "\[Rule]", "1.`"}], "}"}], 
                        "]"}]}], "]"}], "]"}], ",", System`RowBox[
                 {"RIFunctionEvaluateNameless", "[", System`RowBox[
                   {"id2", ",", System`RowBox[{"{", "1.`", "}"}]}], "]"}]}], 
              "}"}]}], ",", System`RowBox[{"{", System`RowBox[
             {System`RowBox[{"{", System`RowBox[{System`RowBox[{"{", "1", 
                    "}"}], "\[Rule]", "x"}], "}"}], ",", System`RowBox[{"{", 
                System`RowBox[{System`RowBox[{"{", "1", "}"}], "\[Rule]", 
                  "1.`"}], "}"}], ",", System`RowBox[{"{", "1.`", "}"}]}], 
            "}"}], ",", System`RowBox[{"{", "}"}], ",", 
          System`RowBox[{"{", "}"}]}], "]"}]}], "]"}]], "Input", 
 System`GeneratedCell -> System`True, System`CellAutoOverwrite -> 
  System`True, System`CellChangeTimes -> {3.681669301483221*^9}]
